...
[DomainRouter] ── 키워드(1차) / LLM(2차) 분류
    ...
    └─ location → [LocationAgent]  ← DB 조회 + LLM 분석 파이프라인 (CHOI)
                  ...
                  └─ CommercialRepository (db/commercial.db)
...
COMMERCIAL_DB_PATH=/data/commercial.db
```
...
| `COMMERCIAL_DB_PATH` | ⬜ | 상권 DB 경로 (Railway Volume 마운트 시 필수, 로컬은 자동 탐지) |

> ⬜ 선택 항목은 미설정 시 해당 플러그인이 오류 대신 안내 메시지를 반환합니다.

### Railway 배포 시 상권 DB(commercial.db) 등록 방법

`commercial.db`(54 MB)는 GitHub 50 MB 제한으로 인해 Git에 포함되지 않습니다.
Railway 배포 환경에서는 **Railway Volume**을 통해 주입합니다.

#### 1단계 — Railway 대시보드에서 Volume 생성

Railway 프로젝트 → 백엔드 서비스 선택 → **Settings** 탭 →
**Volumes** 섹션 → **Add a Volume** 클릭.

| 항목 | 값 |
| ---- | --- |
| Mount Path | `/data` |
| Volume Size | 1 GB (기본값) |

저장 후 서비스를 재배포합니다.

#### 2단계 — Railway CLI로 DB 파일 업로드

```bash
# Railway CLI 설치 (설치되어 있지 않은 경우)
npm install -g @railway/cli

# 로그인
railway login

# 프로젝트 연결 (이미 연결된 경우 생략)
railway link

# DB 파일을 컨테이너 Volume 경로로 복사
railway run --service <서비스명> \
  cp /dev/stdin /data/commercial.db \
  < integrated_PARK/db/commercial.db
```

> `railway run`이 파일 리디렉션을 지원하지 않는 환경에서는 아래 방법 B를 사용합니다.

#### 방법 B — 임시 업로드 스크립트 활용

```bash
# 1. 컨테이너 쉘 진입
railway shell --service <서비스명>

# 2. 쉘 안에서 — Volume 마운트 확인
ls /data   # 비어 있으면 정상

# 쉘에서는 파일을 직접 쓸 수 없으므로 업로드 스크립트를 서비스에 임시 추가:
# integrated_PARK/upload_db.py (아래 참고)
```

```python
# integrated_PARK/upload_db.py  (Railway 배포 후 1회만 실행, 이후 삭제)
# railway run python upload_db.py
import shutil, os
src = "db/commercial.db"          # 빌드 시 포함된 임시 경로
dst = os.getenv("COMMERCIAL_DB_PATH", "/data/commercial.db")
os.makedirs(os.path.dirname(dst), exist_ok=True)
shutil.copy2(src, dst)
print(f"복사 완료: {dst} ({os.path.getsize(dst):,} bytes)")
```

> 이 스크립트를 사용할 경우 `.gitignore`에서 `integrated_PARK/db/commercial.db` 항목을 일시적으로 제거하여 빌드에 포함시킨 후, 복사 완료 후 다시 추가합니다.
...
# "domains": ["admin", "finance", "legal", "location"] 확인

curl -X POST https://<your-app>.railway.app/api/v1/query \
  -H "Content-Type: application/json" \
  -d '{"question": "홍대 카페 상권 분석해줘", "domain": "location"}'
```

#### 로컬 개발 환경 참고

로컬에서는 `COMMERCIAL_DB_PATH`를 설정하지 않아도 됩니다.
`db/repository.py`가 자동으로 `db/commercial.db`(현재 파일 기준 상대경로)를 탐지합니다.
...
| ------ | ------------ | -------------------- |
| `admin` | 영업 신고, 허가, 행정 절차 | SeoulCommercialPlugin |
| `finance` | 매출 시뮬레이션, 투자 분석, 수익성 | FinanceSimulationPlugin |
| `legal` | 법령 해석, 계약, 임대차, 권리 의무 | LegalSearchPlugin |
| `location` | 상권 분석, 지역 비교, 매출 지역 데이터, 창업 입지 | CommercialRepository (SQLite DB) |
...
│   └── location_agent.py         # 상권분석 에이전트 (DB 조회 + LLM 분석, CHOI)
├── db/
│   ├── repository.py             # CommercialRepository — AREA_MAP·INDUSTRY_CODE_MAP 포함
│   └── commercial.db             # 서울시 상권 SQLite DB 54 MB (Git 제외, Railway Volume 주입)
├── plugins/
...
│   └── location_plugin.py           # 상권분석 SK Plugin 래퍼 (CHOI)
├── signoff/
