...
export const LAYER_META = {
   ...
   tourist_info: { icon: "ℹ️",  label: "관광안내소", color: "#FF9800", bg: "#FFF3E0" },
   tourist_spot: { icon: "🏖️", label: "관광지",      color: "#9C27B0", bg: "#F3E5F5" },
...
export function parseWmsProps(p, layerType) {
   ...
   if (layerType === "tourist_info") {
      return {
         ...
         tel:    p.inf_tel || "",
         hours:  p.wws_tme && p.wwe_tme ? `평일 ${p.wws_tme.slice(0,5)}~${p.wwe_tme.slice(0,5)}` : "",
      };
   }
   if (layerType === "tourist_spot") {
      return {
         ...
         tel:    p.mng_tel || p.tel || "",
         hours:  p.wds_tme && p.wde_tme ? `평일 ${p.wds_tme.slice(0,5)}~${p.wde_tme.slice(0,5)}` : "",
      };
   }
   if (layerType === "market") {
      return {
   ...
   return { pnu:"", addr:"", jibun:"", sido:"", sigg:"", dong:"", name:"정보 없음", remark:"", tel:"", hours:"" };
}
...
export async function handleWmsClick(map, coordinate) {
   const wmsLayers = map.getLayers().getArray().filter((l) =>
      ["cadastral", "tourist_info", "tourist_spot", "market"].includes(l.get("name"))
   );
   ...
   for (const wmsLayer of wmsLayers) {
      ...
      try {
         ...
         let feat = null;
         try { feat = JSON.parse(text).features?.[0]; } catch { /* ignore */ }
         if (!feat) continue;
         ...
         return { parsed: { ...parsed, type: layerType }, layerType, landValue };
      } catch (err) {
