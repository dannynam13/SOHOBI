...
import { useDongLayer, DONG_STYLE_DEFAULT, DONG_STYLE_HOVER, DONG_STYLE_SELECTED } from "../hooks/useDongLayer";
import { handleWmsClick } from "../hooks/useWmsClick";
// ── 상수/스타일 ────────────────────────────────────────────────
...
export default function MapView() {
   ...
   const clearAll = () => {
      clearMarkers();
      setPopup(null); setKakaoDetail(null); setNearbyCount(null);
      setLandValue(null); setWmsPopup(null); setDongPanel(null);
      setCatCounts({});
   };
   ...
   useEffect(() => {
      ...
      (async () => {
         try {
            ...
            if (region?.region_2depth_name) currentGuNmRef.current = region.region_2depth_name;
         } catch { /* ignore */ }
      })();
      ...
      const clickHandler = async (e) => {
         if (dongModeRef.current !== "none") {
            ...
            const feat   = bLayer?.getSource?.()?.getFeatures
               ? map.forEachFeatureAtPixel(e.pixel, f => f, { layerFilter: l => l===bLayer, hitTolerance:6 })
               : null;
               ...
               if (_dongNm) {
                  ...
                  try {
                     if (_mode === "sales") {
                        ...
                        const jj = await rr.json();
                        if (jj) setDongPanel({ mode:_mode, dongNm:_dongNm, admNm:_admNm, guNm:_guNm, admCd:_admCd, apiData:jj });
                     }
                  } catch (err) { console.error("[동 클릭 패널]", err); }
         ...
         if (feature?.get("store")) {
            ...
            setPopup(store); setKakaoDetail(null); setLoadingDetail(true);
            const detail = await fetchKakaoDetail(store.상호명, store.도로명주소);
            setKakaoDetail(detail); setLoadingDetail(false);
            return;
         }
         ...
         setLoading(true); setPopup(null); setKakaoDetail(null);
         try {
            const res    = await fetch(`${FASTAPI_URL}/map/nearby?lat=${lat}&lng=${lng}&radius=${radius}&limit=${limit}`);
            const data   = await res.json();
   ...
   return (
      <div className="mv-root">
         ...
         <MapControls
            ...
            nearbyCount={nearbyCount} loading={loading} onClear={clearAll}
            dongMode={dongMode} onDongMode={handleDongMode}
            dongLoading={dongLoading} currentGuNm={currentGuNmRef.current}
         />
         <button className="mv-layer-btn" onClick={() => setShowPanel((p) => !p)}>🗂️</button>
         ...
         <WmsPopup  wmsPopup={wmsPopup} landValue={landValue} onClose={() => setWmsPopup(null)} />
