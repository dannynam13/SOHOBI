...
// ── 업종별 매출 패널 ──────────────────────────────────────────
const SVC_COLOR = {
   I2:"#FF6B6B", G2:"#FF9800", S2:"#4ecdc4", L1:"#2196F3",
   I1:"#9C27B0", P1:"#F59E0B", Q1:"#E03131", R1:"#2F9E44",
   M1:"#1971C2", N1:"#607D8B",
};
const SVC_LABEL = {
   I2:"음식", G2:"소매", S2:"수리·개인", L1:"부동산",
   I1:"숙박", P1:"교육", Q1:"의료", R1:"스포츠",
   M1:"전문·기술", N1:"시설관리",
};
...
function SvcPanel({ svcData }) {
   ...
   const total = svcData.reduce((s, r) => s + (Number(r.tot_sales_amt) || 0), 0);
   if (total === 0) return null;
   return (
      <div style={{ background:"#f8fafc", borderRadius:12, padding:14 }}>
         <div style={{ fontSize:11, fontWeight:700, color:"#475569", marginBottom:10 }}>🏪 업종별 매출</div>
         {svcData.map((r) => {
            ...
            return (
               <div key={r.svc_cd} style={{ marginBottom:9 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:3 }}>
                     ...
                     <span style={{ color:col, fontWeight:700 }}>{formatAmt(amt)} ({pct}%)</span>
                  </div>
                  <div style={{ height:6, background:"#e2e8f0", borderRadius:3, overflow:"hidden" }}>
   ...
   return (
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
         ...
         <div style={{ background:"#f5f3ff", borderRadius:12, padding:14 }}>
            <div style={{ fontSize:10, color:"#888", marginBottom:4 }}>총 점포수</div>
            <div style={{ fontSize:24, fontWeight:800, color:"#7C3AED" }}>{Math.round(total)}개</div>
         </div>
         {/* 업종별 점포수 */}
         <div style={{ background:"#f8fafc", borderRadius:12, padding:14 }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#475569", marginBottom:10 }}>🏪 업종별 점포수</div>
            {rows.map((r) => {
               ...
               return (
                  <div key={r.svc_cd} style={{ marginBottom:9 }}>
                     <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:3 }}>
                        <span style={{ fontWeight:600 }}>{lbl}</span>
                        <span style={{ color:col, fontWeight:700 }}>{Math.round(cnt)}개 ({pct}%)</span>
                     </div>
                     <div style={{ height:6, background:"#e2e8f0", borderRadius:3, overflow:"hidden" }}>
                        <div style={{ width:`${pct}%`, height:"100%", background:col, borderRadius:3 }} />
                     </div>
                     ...
                     <div style={{ display:"flex", gap:10, marginTop:3, fontSize:10, color:"#94a3b8" }}>
                        <span>개업률 <span style={{ color:"#059669", fontWeight:700 }}>{r.opbiz_rt}%</span></span>
                        <span>폐업률 <span style={{ color:"#dc2626", fontWeight:700 }}>{r.clsbiz_rt}%</span></span>
                        {r.frc_stor_co > 0 && <span>프랜차이즈 <span style={{ color:"#7C3AED", fontWeight:700 }}>{Math.round(r.frc_stor_co)}개</span></span>}
                     </div>
                  </div>
               );
            })}
   ...
   return (
      <div style={{ display:"flex", alignItems:"center", gap:14 }}>
         <svg width="90" height="90" viewBox="0 0 100 100">
            {/* 여성 (배경) */}
            <circle cx={CX} cy={CY} r={R} fill="none" stroke="#fce7f3" strokeWidth="14"/>
            {/* 남성 */}
            ...
            <circle cx={CX} cy={CY} r={R} fill="none"
               ...
               strokeLinecap="round"
            />
            <text x={CX} y={CY - 5} textAnchor="middle" fontSize="13" fontWeight="700" fill="#1e293b">{malePct}%</text>
            <text x={CX} y={CY + 10} textAnchor="middle" fontSize="9" fill="#64748b">남성</text>
         </svg>
         <div style={{ display:"flex", flexDirection:"column", gap:8, flex:1 }}>
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
               ...
               <span style={{ marginLeft:"auto", fontWeight:700, fontSize:12, color:"#2563eb" }}>{malePct}% ({formatAmt(male)})</span>
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
               <div style={{ width:10, height:10, borderRadius:"50%", background:"#ec4899", flexShrink:0 }}/>
               <span style={{ fontSize:12 }}>여성</span>
               <span style={{ marginLeft:"auto", fontWeight:700, fontSize:12, color:"#ec4899" }}>{femalePct}% ({formatAmt(female)})</span>
            </div>
         </div>
      </div>
   );
   ...
   return (
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
         ...
         {(d.sales_male || d.sales_female) && (
            <div style={{ background:"#f8fafc", borderRadius:12, padding:14 }}>
               <div style={{ fontSize:11, fontWeight:700, color:"#475569", marginBottom:10 }}>👤 성별 매출</div>
         ...
         {(d.age20 || d.age30 || d.age40 || d.age50) && (
            <div style={{ background:"#f8fafc", borderRadius:12, padding:14 }}>
               ...
               {[["20대",d.age20,"#6366f1"],["30대",d.age30,"#0ea5e9"],["40대",d.age40,"#10b981"],["50대",d.age50,"#f59e0b"]].map(([label,val,col]) => {
...
// ── 메인 컴포넌트 ─────────────────────────────────────────────
export default function DongPanel({ dongPanel, onClose, quarters = [], selectedQuarter, onQuarterChange, svcData, selectedSvc, onSvcChange }) {
   ...
   return (
      <div className="mv-dong-panel">
         <div className="mv-dong-panel__header" style={{ background: panelColor }}>
            <div className="mv-dong-panel__header-row">
               <div>
                  ...
                  <div className="mv-dong-panel__name">{dongPanel.dongNm || dongPanel.admNm}</div>
               </div>
               <button onClick={onClose} className="mv-dong-panel__close">✕</button>
            </div>
            <div className="mv-dong-panel__mode-label">
               {isRE ? "🏢 실거래가 분석" : isStore ? "🏪 점포수 분석" : "📊 상권 매출 분석"}
            </div>
         </div>
         ...
         {svcData && svcData.length > 0 && !isRE && !isStore && (
            <div style={{ padding:"6px 12px", background:"#f8fafc", borderBottom:"1px solid #e5e7eb", display:"flex", alignItems:"center", gap:8 }}>
               <span style={{ fontSize:11, color:"#666", flexShrink:0 }}>🏪 업종</span>
               <select
                  value={selectedSvc || ""}
         ...
         <div className="mv-dong-panel__body">
            {!d ? (
               ...
               <>
                  {/* ① 총매출 / 평균比 */}
                  <SalesSummary d={d} panelColor={panelColor} panelBg={panelBg} avg={dongPanel.avg} />
