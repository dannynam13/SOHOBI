...
class SeoulRtmsDAO(BaseDAO):
    ...
    def get_law_cds_by_adm_cd(self, adm_cd: str) -> list:
        ...
        rows = self._query(
            "SELECT DISTINCT M.LAW_CD, L.GU_NM FROM LAW_ADM_MAP M "
            "JOIN LAW_DONG_SEOUL L ON M.LAW_CD = L.LAW_CD "
            "WHERE M.ADM_CD = :1",
            [adm_cd]
        )
        return [{"law_cd": r[0], "gu_nm": r[1]} for r in rows]
    ...
    async def fetch_by_law_cd(
        ...
        """
        법정동코드 기준 실거래 조회 (구 전체 당겨서 법정동 필터)
        years_back: 최근 N년치
        """
        now = datetime.now()
        ...
        async with httpx.AsyncClient() as client:
            tasks = []
            for yr in years:
                filters = {"CGG_NM": gu_nm, "RCPT_YR": yr}
                if rtms_type:
                    filters["RTMS_TPCD"] = rtms_type
        ...
        filtered = [
            r for r in all_rows
            if (r.findtext('STDG_CD') or '').strip() == law_cd
        ]
